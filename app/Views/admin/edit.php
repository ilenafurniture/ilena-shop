                ...
                <table class="table-input w-100">
                    <tbody>
                                ...
                                <div class="baris">
                                    ...
                                    <select name="kategori" onchange="generateId(event)">
                                        <?php foreach ($koleksi as $ind_k => $k) { ?>
                                            <option value="<?= $k['id']; ?>" <?= $k['nama'] == $produk['kategori'] ? 'selected' : ''; ?>>
                                                <?= $k['nama']; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                ...
                                <div class="baris">
                                    ...
                                    <select name="subkategori" onchange="generateId(event)">
                                        <?php foreach ($jenis as $ind_k => $k) { ?>
                                            <option value="<?= $k['id']; ?>" <?= $k['nama'] == $produk['subkategori'] ? 'selected' : ''; ?>>
                                                <?= $k['nama']; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                        ...
                        <tr class="d-none">
                            <td>ID Produk</td>
                            <td>
                                <div class="baris"><input type="text" class="form-control" name="id" required value="<?= $produk['id']; ?>">
                                </div>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="baris">
                                    <div>
                                        <input type="checkbox" name="ruang_tamu" id="check_tamu" <?= $produk['ruang_tamu'] ? 'checked' : ''; ?>>
                                        <label for="check_tamu">Ruang Tamu</label>
                                    </div>
                                    <div>
                                        <input type="checkbox" name="ruang_keluarga" id="check_keluarga" <?= $produk['ruang_keluarga'] ? 'checked' : ''; ?>>
                                        <label for="check_keluarga">Ruang Keluarga</label>
                                    </div>
                                    <div>
                                        <input type="checkbox" name="ruang_tidur" id="check_tidur" <?= $produk['ruang_tidur'] ? 'checked' : ''; ?>>
                            ...
                            <div class="container-gambar" id="container-gambar<?= $ind_v + 1; ?>">
                                <div id="container-input-gambar<?= $ind_v + 1; ?>">
                                    <?php foreach (explode(",", $v['urutan_gambar']) as $ind_urutanGambar => $urutanGambar) { ?>
                                        <div>
                                            <input type="file" id="input-gambar-<?= $ind_v + 1; ?>-<?= $ind_urutanGambar + 1; ?>" name="gambar-<?= $ind_v + 1; ?>-<?= $ind_urutanGambar + 1; ?>" style="display: none;" onchange="uploadFile(event)">
                                            <label for="input-gambar-<?= $ind_v + 1; ?>-<?= $ind_urutanGambar + 1; ?>" class="btn-default">+</label>
                                        </div>
                                    <?php } ?>
                                    <div>
                                        <input type="file" id="input-gambar-<?= $ind_v + 1; ?>-<?= count(explode(",", $v['urutan_gambar'])) + 1; ?>" name="gambar-<?= $ind_v + 1; ?>-<?= count(explode(",", $v['urutan_gambar'])) + 1; ?>" style="display: none;" onchange="uploadFile(event)">
                                        <label for="input-gambar-<?= $ind_v + 1; ?>-<?= count(explode(",", $v['urutan_gambar'])) + 1; ?>" class="btn-default">+</label>
                                    </div>
                                </div>
                                <?php foreach (explode(",", $v['urutan_gambar']) as $ind_u => $u) { ?>
                                    <div class="item-gambar" onclick="hapusSubvarian('<?= $ind_v + 1; ?>','<?= $ind_u + 1; ?>',event)">
                                        <p>X</p>
