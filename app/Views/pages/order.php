...
<div class="container-offcanvas" style="z-index: 100;">
    <div class="d-flex justify-content-end"><button class="btn btn-light" onclick="closeDetail()"><i
...
function openDetail(indP) {
    ...
    itemElm.innerHTML =
        '<div class="d-flex mb-1 border-bottom pb-1"><p class="d-block fw-bold m-0 text-center text-secondary" style="flex: 1; letter-spacing: -1px;">Nama</p><p class="d-block fw-bold m-0 text-center text-secondary" style="flex: 1; letter-spacing: -1px;">Jumlah</p><p class="d-block fw-bold m-0 text-center text-secondary" style="flex: 1; letter-spacing: -1px;">Harga</p></div>';
    pesananSelected.items.forEach(element => {
        if (element.name != 'Biaya Ongkir' && element.name != 'Biaya Admin') {
            itemElm.innerHTML +=
                '<div class="d-flex py-1"><p class="d-block fw-bold m-0 text-center" style="flex: 1; letter-spacing: -1px;">' +
                element.name +
    ...
    btnDetailElm.href = `/orderdetail/${pesananSelected.status.toLowerCase()}?idorder=${pesananSelected.id_midtrans}`;
